...
export type CopardBetsInRealTimeModalProps = {
  onClose: () => void;
  editBet: BetInRealTimeType | undefined;
  refetch: () => void;
};

const copardBetsInRealTimeSchema = z.object({
...
export const CopardBetsInRealTimeEditModal: FC<
  CopardBetsInRealTimeModalProps
> = ({ editBet, onClose, refetch }) => {
  ...
  const form = useForm({
    ...
    validators: {
      onChange: copardBetsInRealTimeSchema,
    },
    onSubmit: ({ value }) =>
      mutation.mutate({
        min: value.min,
