...
export type IAAIfinalBetRangeModalProps = {
  onClose: () => void;
  editRange: FinalBetRangeType | undefined;
  refetch: () => void;
};

const IAAIfinalBetRangeSchema = z.object({
...
export const IAAIfinalBetRangeEditModal: FC<IAAIfinalBetRangeModalProps> = ({
  editRange,
  onClose,
  refetch,
}) => {
  ...
  const form = useForm({
    ...
    validators: {
      onChange: IAAIfinalBetRangeSchema,
    },
    onSubmit: ({ value }) =>
      mutation.mutate({
        min: value.min,
