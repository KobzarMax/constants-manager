...
const IAAIconstantSchema = z.object({
  value: z
    .number()
...
export const IAAIConstantForm: FC = () => {
  const {
    data: IAAIConstantData,
    isLoading,
    refetch,
  } = useQuery({
    queryKey: ["IAAIConstant"],
    queryFn: async () => {
  ...
  const form = useForm({
    defaultValues: { value: IAAIConstantData?.value ?? 0 },
    onSubmit: ({ value }) => mutation.mutate(value.value),
    validators: {
      onChange: IAAIconstantSchema,
    },
  });
        ...
        <div className="mb-4">
          <label htmlFor="value" className="block font-semibold text-lg mb-1">
            ...
            <span className="text-xs font-normal">
              Last update: {formatTimestamp(IAAIConstantData?.updated_at)}
            </span>
          </label>
